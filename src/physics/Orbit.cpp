...
Orbit::Orbit()
    : a(7000000.0), e(0.0), i(0.0), Omega(0.0), omega(0.0), nu(0.0),
      mu(Constants::MU_EARTH), epoch(0.0) {}
...
Orbit::Orbit(double a_, double e_, double i_, double Omega_, double omega_,
    ...
    : a(a_), e(e_), i(i_), Omega(Omega_), omega(omega_), nu(nu_),
      mu(mu_), epoch(epoch_) {
    if (a <= 0.0) throw std::invalid_argument("Semieje mayor debe ser positivo");
    if (e < 0.0 || e >= 2.0) throw std::invalid_argument("Excentricidad inválida");
}
...
Orbit::Orbit(const dvec3& r, const dvec3& v, double mu_, double epoch_)
    ...
    if (rmag < 1e-6) throw std::invalid_argument("Posición muy cercana al origen");
    ...
    // Anomalía verdadera
    double cos_nu = glm::dot(e_vec, r) / (e * rmag);
    cos_nu = glm::clamp(cos_nu, -1.0, 1.0);
    
    nu = std::acos(cos_nu);
    if (glm::dot(r, v) < 0) nu = 2.0 * M_PI - nu;
}
...
dvec3 Orbit::getPositionAtTime(double t) const {
    ...
    // Anomalía media
    double M = std::fmod(n * dt, 2.0 * M_PI);
